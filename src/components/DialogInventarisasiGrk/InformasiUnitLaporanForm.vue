...
import { blueColorLib, greenColorLib, neutralColorLib } from '@/utils/constant.js'

defineOptions({
  name: 'DialogFormInformasiUnitLaporanComponent',
})
...
watch(
  () => props.params.isUnitLaporanSendiri,
  (newValue) => {
    if (newValue) {
      isHoverUnitSendiri.value = true
      isHoverUnitLain.value = false
    } else {
      isHoverUnitSendiri.value = false
      isHoverUnitLain.value = true
    }
  },
  { deep: true }
)
