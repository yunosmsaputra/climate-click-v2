...
defineOptions({
  name: 'DialogDetailRiwayatLaporan',
})
...
<template>
  <div class="bg-netral-white pt-[18px] pr-[18px] pb-6 pl-6">
    ...
    <div v-if="isExpand">
      ...
      >
        <template #date>{{ moment(item.updated_date).format('DD MMM YYYY') }}</template>
        <template #time>{{ moment(item.updated_date).format('HH:mm') }}</template>
        <template #content>
          ...
          <div class="text-netral-textField text-s mt-1">({{ item.keterangan }})</div>
        </template>
      </HistoryData>
    </div>
  </div>
