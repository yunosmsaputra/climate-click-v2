...
export const useFugitiveAparDialogActions = () => {
  ...
  const openDialogForm = (idEditData = null) => {
    ...
    if (idEditData) {
      ...
      callApi({
        config: {
          url: `${host.grk}/api/v2/fugitive-apar/detail`,
          method: 'GET',
          params: {
            id: idEditData,
          },
        ...
        onSuccess: (response) => {
          paramsForm.value = {
            ...
            data_list: response.data.data.data_list.map((item) => ({
              ...item,
              penyimpanan_apar_fixed: convertNumberForSave(item.penyimpanan_apar_fixed),
              penyimpanan_apar_portable: convertNumberForSave(item.penyimpanan_apar_portable),
              penggunaan_apar: convertNumberForSave(item.penggunaan_apar),
              jenis_gas: {
                id: item.id_faktor_emisi,
                nama: item.nama_faktor_emisi,
              },
            })),
      ...
      callApi({
        ...
        onSuccess: (response) => {
          ...
          const dataList = paramsForm.value.data_list.map((item) => ({
            id_faktor_emisi: item.id_faktor_emisi,
            nama_faktor_emisi: item.nama_faktor_emisi,
            penyimpanan_apar_fixed: convertNumberForSave(item.penyimpanan_apar_fixed),
            penyimpanan_apar_portable: convertNumberForSave(item.penyimpanan_apar_portable),
            penggunaan_apar: convertNumberForSave(item.penggunaan_apar)
          }))
          callApi({
            config: {
              url: `${host.grk}/api/v2/fugitive-apar/preview-hitung`,
              method: 'POST',
              data: {
                id_unit: userProfile.unit.id,
  ...
  const onSaveData = (filePath = null) => {
    const dataList = paramsForm.value.data_list.map((item) => ({
      id_faktor_emisi: item.jenis_gas.id,
      nama_faktor_emisi: item.jenis_gas.nama,
      penyimpanan_apar_fixed: convertNumberForSave(item.penyimpanan_apar_fixed),
      penyimpanan_apar_portable: convertNumberForSave(item.penyimpanan_apar_portable),
      penggunaan_apar: convertNumberForSave(item.penggunaan_apar)
    }))
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/save`,
        method: 'POST',
        data: {
  ...
  const getRiwayatData = (data) => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/history`,
        method: 'GET',
        params: {
          id: data.id,
        },
  ...
  const sendLaporan = (fileUrl = null) => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/kirim`,
        method: 'POST',
        data: {
          ...(paramsForm.value.id && { id: paramsForm.value.id }),
          id_unit: userProfile.unit.id,
  ...
  const approveLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/approve-ui-sh`,
        method: 'POST',
        data: {
  ...
  const deleteData = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/delete`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
        },
  ...
  const rejectLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/tolak-ui-sh`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
  ...
  const revisiLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/revisi-divtek`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
          keterangan_tolak: keteranganPenolakan.value,
