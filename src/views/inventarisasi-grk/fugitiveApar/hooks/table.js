...
import { blueColorLib, greenColorLib, host, monthFull, redColorLib } from '@/utils/constant.js'
...
export const useFugitiveAparTableAllActions = () => {
  ...
  const columns = ref([
    ...
    {
      key: 'total_penyimpanan_apar',
      title: 'Total Penyimpanan APAR (kgCO₂)',
      sort: true,
      clickRow: true,
      // width: '214px',
      render: (val) => {
        return h(
          ...
          {},
          {
            default: () => konversiRibuan(val.total_penyimpanan_apar),
          }
        )
      },
    },
    {
      key: 'total_penggunaan_apar',
      title: 'Total Penggunaan APAR (kgCO₂)',
      sort: true,
      clickRow: true,
      // width: '214px',
      render: (val) => {
        return h(
          ...
          {},
          {
            default: () => konversiRibuan(val.total_penggunaan_apar),
          }
        )
      },
    },
  ...
  const getData = (page = 1, size = 10, activeTab = 'Semua Laporan') => {
    loading.value = true
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-apar/page`,
        method: 'GET',
        params: {
          page: page - 1,
          size: size,
          sort: 'updatedDate,desc',
          q: `tahun==${periode.value}${
            activeTab === 'Draft Laporan'
              ? `;status=in=('DRAFT','DITOLAK_OLEH_UI_SH','PERMINTAAN_REVISI_DIVTEK')`
              : activeTab === 'Persetujuan Laporan'
