...
export const useFugitiveAcRuanganTableActions = () => {
  ...
  const columns = ref([
    ...
    },
    {
      ...
      render: (val) => {
        return h(
          ...
          {},
          {
            default: () => konversiRibuan(val.total_konsumsi_bb),
          }
        )
      },
    },
    {
      key: 'total_emisi',
      title: 'Total Emisi (tCO<sub>2</sub>e)',
      sort: true,
      clickRow: true,
      width: '214px',
      render: (val) => {
        return h(
          ...
          {},
          {
            default: () => konversiRibuan(val.total_emisi / 1000),
          }
        )
      },
    },
  ...
  const getData = (page = 1, size = 10, activeTab = 'Semua Laporan', sort = 'updatedDate,desc') => {
    loading.value = true
    callApi({
      config: {
        url: `${host.grk}/api/v2/mck/page`,
        method: 'GET',
        params: {
          page: page - 1,
          size: size,
  ...
  const getCounterPersetujuan = () => {
    callApi({
      config: {
        url: `${host.grk}/api/v2/mck/list`,
        method: 'GET',
        params: {
          q: `status==${userProfile.role === 'DIVTEK' ? 'MENUNGGU_PERSETUJUAN_DIVTEK' : 'MENUNGGU_PERSETUJUAN_UI_SH'}`,
        },
      },
      onSuccess: (response) => {
