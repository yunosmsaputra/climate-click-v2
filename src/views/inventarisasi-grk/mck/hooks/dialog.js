...
export const useFugitiveAcRuanganDialogActions = () => {
  ...
  const openDialogForm = (idEditData = null) => {
    ...
    if (idEditData) {
      ...
      callApi({
        config: {
          url: `${host.grk}/api/v2/mck/detail`,
          method: 'GET',
          params: {
            id: idEditData,
          },
        ...
        onSuccess: (response) => {
          paramsForm.value = {
            ...
            data_list: response.data.data.data_list.map((item) => ({
              ...item,
              konsumsi_bb: convertNumberToForm(item.konsumsi_bb),
              faktor_emisi: { id: item.id_faktor_emisi, nama: item.nama_faktor_emisi },
              status_kepemilikan: item.status_kepemilikan == 'SEWA' ? 'Sewa' : 'Milik PLN Group',
            })),
      ...
      callApi({
        ...
        onSuccess: (response) => {
          ...
          const dataList = paramsForm.value.data_list.map((item) => ({
            status_kepemilikan: item.status_kepemilikan == 'Sewa' ? 'SEWA' : 'MILIK_PLN_GROUP',
            id_faktor_emisi: item.faktor_emisi.id,
            konsumsi_bb: convertNumberForSave(item.konsumsi_bb),
          }))
          callApi({
            config: {
              url: `${host.grk}/api/v2/mck/preview-hitung`,
              method: 'POST',
              data: {
                id_unit: userProfile.unit.id,
  ...
  const getRiwayatData = (data) => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/mck/history`,
        method: 'GET',
        params: {
          id: data.id,
        },
  ...
  const approveLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/mck/approve-ui-sh`,
        method: 'POST',
        data: {
  ...
  const deleteData = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/mck/delete`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
        },
  ...
  const rejectLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/mck/tolak-ui-sh`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
  ...
  const revisiLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/mck/revisi-divtek`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
          keterangan_tolak: keteranganPenolakan.value,
