...
export const useFugitiveAcRuanganTableActions = () => {
  ...
  const columns = ref([
    ...
    {
      key: 'total_penggunaan',
      title: 'Total Penggunaan (kg)',
      sort: true,
      clickRow: true,
      render: (val) => {
        return h(
          ...
          {},
          {
            default: () => konversiRibuan(val.total_penggunaan),
          }
        )
      },
    },
  ...
  const getData = (page = 1, size = 10, activeTab = 'Semua Laporan', sort = 'updatedDate,desc') => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kulkas/page`,
        method: 'GET',
        params: {
          page: page - 1,
          size: size,
  ...
  const getCounterPersetujuan = () => {
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kulkas/list`,
        method: 'GET',
        params: {
