  ...
  },
  {
    ...
    render: (data, index) => {
      return h(
        ...
        {},
        {
          default: () => (data.penggunaan ? konversiRibuan(data.penggunaan) : '-'),
        }
      )
    },
  },
            ...
            {{
              konversiRibuan(
                data.data_list?.reduce(
                  (sum, item) =>
                    sum +
                    (Number.isNaN(parseFloat(item.penggunaan)) ? 0 : parseFloat(item.penggunaan)),
                  0
                )
              )
            }}
            ...
            {{
              konversiRibuan(
                data.data_list?.reduce(
                  (sum, item) =>
                    sum +
                    (Number.isNaN(parseFloat(item.emisi_co2e)) ? 0 : parseFloat(item.emisi_co2e)),
                  0
                )
              )
            }}
