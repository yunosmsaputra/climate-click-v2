...
export const useFugitiveAcKendaraanDialogActions = () => {
  ...
  const openDialogForm = (idEditData = null) => {
    ...
    if (idEditData) {
      ...
      callApi({
        config: {
          url: `${host.grk}/api/v2/fugitive-ac-kendaraan/detail`,
          method: 'GET',
          params: {
            id: idEditData,
          },
        ...
        onSuccess: (response) => {
          paramsForm.value = {
            ...
            ...(response.data.data.data_list.length > 0
              ? {
                  data_list: response.data.data.data_list.map((item) => ({
                    ...item,
                    penggunaan: convertNumberToForm(item.penggunaan),
                    faktor_emisi: { id: item.idFaktorEmisi, nama: item.namaFaktorEmisi },
                  })),
                }
              : {
                  data_list: [{}],
    ...
    if (refForm.value.validate()) {
      ...
      callApi({
        config: {
          ...
          method: 'GET',
          params: {
            id: paramsForm.value.isUnitLaporanSendiri
              ? userProfile.unit.id
        ...
        onSuccess: (response) => {
          ...
          const dataList = paramsForm.value.data_list.map((item) => ({
            ...
            idFaktorEmisi: item.faktor_emisi.id,
          }))
          callApi({
            config: {
              url: `${host.grk}/api/v2/fugitive-ac-kendaraan/preview-hitung`,
              method: 'POST',
              data: {
            ...
            onSuccess: (resp) => {
              ...
              if (file) {
                ...
                paramsForm.value = {
                  ...resp.data.data,
                  isUnitLaporanSendiri: paramsTemp.isUnitLaporanSendiri,
                  ...(paramsTemp.id && { id: paramsTemp.id }),
                  periode: paramsTemp.periode,
                  file: file,
                  ...
                  unitChild: paramsTemp.unitChild,
                }
  ...
  const onSaveData = (filePath = null) => {
    const isHaveDataList = paramsForm.value.data_list.every((item) => {
      return item.penggunaan !== undefined || item.faktor_emisi !== undefined
    })
    loadingButton.value = true
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kendaraan/save`,
        method: 'POST',
        data: {
          id_unit: paramsForm.value.isUnitLaporanSendiri
            ? userProfile.unit.id
          ...
          tahun: paramsForm.value.periode.year,
          ...(isHaveDataList && {
            data_list: paramsForm.value.data_list.map((item) => ({
              penggunaan: item.penggunaan ? convertNumberForSave(item.penggunaan) : null,
              idFaktorEmisi: item.faktor_emisi?.id
                ? item.faktor_emisi.id
                : (item.idFaktorEmisi ?? null),
            })),
          }),
          ...(paramsForm.value.sumber_data && {
            sumber_data: paramsForm.value.sumber_data,
    ...
    callApi({
      ...
      onSuccess: (response) => {
        ...
        if (
          ...
          const fileExtension =
            dataDetail.value.file_evidence_url.split('.')[
              dataDetail.value.file_evidence_url.split('.').length - 1
            ]
          getPreviewFile(dataDetail.value, fileExtension)
        } else {
  ...
  const getPreviewFile = (data, fileExtension = 'pdf') => {
    loadingDialog.value = true
    callApi({
      config: {
        url: `${host.api}/api/minio/view`,
  ...
  const getRiwayatData = (data) => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kendaraan/history`,
        method: 'GET',
        params: {
          id: data.id,
        },
  ...
  const backToForm = (params) => {
    ...
    paramsForm.value = {
      ...params,
      data_list: params.data_list.map((item) => ({
        ...item,
        faktor_emisi: { id: item.idFaktorEmisi, nama: item.namaFaktorEmisi },
      })),
  ...
  const sendLaporan = (fileUrl = null) => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kendaraan/kirim`,
        method: 'POST',
        data: {
          ...(paramsForm.value.id && { id: paramsForm.value.id }),
          id_unit: userProfile.unit.id,
      ...
      onSuccess: () => {
        ...
        summaryDialogAlert.value = {
          ...
          message: 'Laporan yang anda buat telah berhasil dikirim ke UI/SH Pusat',
          isSuccess: true,
          buttonText: 'Arahkan Halaman Pelaporan',
        }
        showDialogAlert.value = true
  ...
  const approveLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kendaraan/approve-ui-sh`,
        method: 'POST',
        data: {
  ...
  const deleteData = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kendaraan/delete`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
        },
  ...
  const rejectLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kendaraan/tolak-ui-sh`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
  ...
  const revisiLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/fugitive-ac-kendaraan/revisi-divtek`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
          keterangan_tolak: keteranganPenolakan.value,
