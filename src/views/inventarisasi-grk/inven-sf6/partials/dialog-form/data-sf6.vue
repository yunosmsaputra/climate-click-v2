...
defineOptions({
  name: 'InvenSf6DialogFormDataSf6',
})
...
<template>
  <div class="pt-[18px] pr-[18px] pb-6 pl-6 bg-netral-white">
    <TitleSection>Data SF6</TitleSection>
    <div class="mt-4" v-for="(item, index) in params.data_list" :key="index">
      ...
      <AmFormRow class="flex gap-x-4 mt-2">
        <AmFormCol class="w-[240px]">
          <AmFormLabel required size="medium">Jumlah SF6</AmFormLabel>
          <div class="text-xs leading-4 text-netral-textDisabled mt-[6px]">
            Berisikan Jumlah SF 6 yang disimpan dan digunakan
          </div>
        </AmFormCol>
        <AmFormCol class="w-[calc((100%-272px)/2)]">
          <AmTextField
            v-model="params.data_list[index].sf6_disimpan"
            placeholder="SF6 yang Disimpan"
            size="medium"
        ...
        <AmFormCol class="w-[calc((100%-272px)/2)]">
          <AmTextField
            v-model="params.data_list[index].sf6_digunakan"
            placeholder="SF6 yang Digunakan"
            size="medium"
            rules="required|number|min:1"
            formattedNumber
            :is-loading="isLoading"
