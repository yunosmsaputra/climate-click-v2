...
export const useInvenSf6DialogActions = () => {
  ...
  const openDialogForm = (idEditData = null) => {
    ...
    if (idEditData) {
      ...
      callApi({
        config: {
          url: `${host.grk}/api/v2/inven-sf6/detail`,
          method: 'GET',
          params: {
            id: idEditData,
          },
        ...
        onSuccess: (response) => {
          paramsForm.value = {
            ...
            data_list: response.data.data.data_list.map((item) => ({
              ...item,
              sf6_disimpan: convertNumberToForm(item.sf6_disimpan),
              sf6_digunakan: convertNumberToForm(item.sf6_digunakan),
            })),
      ...
      callApi({
        ...
        onSuccess: (response) => {
          ...
          const dataList = paramsForm.value.data_list.map((item) => ({
            sf6_digunakan: convertNumberForSave(item.sf6_digunakan),
            sf6_disimpan: convertNumberForSave(item.sf6_disimpan),
          }))
          callApi({
            config: {
              url: `${host.grk}/api/v2/inven-sf6/preview-hitung`,
              method: 'POST',
              data: {
  ...
  const onSaveData = (filePath = null) => {
    const isHaveDataList = paramsForm.value.data_list.every((item) => {
      return item.sf6_digunakan !== undefined || item.sf6_disimpan !== undefined
    })
    const dataList = paramsForm.value.data_list.map((item) => ({
      sf6_digunakan: item.sf6_digunakan ? convertNumberForSave(item.sf6_digunakan) : null,
      sf6_disimpan: item.sf6_disimpan ? convertNumberForSave(item.sf6_disimpan) : null,
    }))
    loadingButton.value = true
    callApi({
      config: {
        url: `${host.grk}/api/v2/inven-sf6/save`,
        method: 'POST',
        data: {
          id_unit: paramsForm.value.isUnitLaporanSendiri
            ? userProfile.unit.id
  ...
  const getRiwayatData = (data) => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/inven-sf6/history`,
        method: 'GET',
        params: {
          id: data.id,
        },
  ...
  const sendLaporan = (fileUrl = null) => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/inven-sf6/kirim`,
        method: 'POST',
        data: {
          ...(paramsForm.value.id && { id: paramsForm.value.id }),
          id_unit: userProfile.unit.id,
  ...
  const approveLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/inven-sf6/approve-ui-sh`,
        method: 'POST',
        data: {
  ...
  const deleteFile = (isDeleteData = false, isReplaceFile = false) => {
    setLoading(true)
    callApi({
      config: {
        url: `${host.api}/api/minio/delete`,
  ...
  const deleteData = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/inven-sf6/delete`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
        },
  ...
  const rejectLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/inven-sf6/tolak-ui-sh`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
  ...
  const revisiLaporan = () => {
    ...
    callApi({
      config: {
        url: `${host.grk}/api/v2/inven-sf6/revisi-divtek`,
        method: 'POST',
        data: {
          id: paramsForm.value.id,
          keterangan_tolak: keteranganPenolakan.value,
